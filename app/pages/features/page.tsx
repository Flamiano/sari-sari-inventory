...
export const TABS = [
    { id: "dashboard", label: "Dashboard", icon: <LayoutDashboard size={16} />, color: "#3B82F6", bg: "rgba(59,130,246,0.1)", desc: "Real-time store overview" },
    { id: "analytics", label: "Analytics", icon: <BarChart3 size={16} />, color: "#8B5CF6", bg: "rgba(139,92,246,0.1)", desc: "Revenue & profit insights" },
    { id: "inventory", label: "Inventory", icon: <Package size={16} />, color: "#10B981", bg: "rgba(16,185,129,0.1)", desc: "Product & stock control" },
    { id: "pos", label: "Point of Sale", icon: <ShoppingCart size={16} />, color: "#F59E0B", bg: "rgba(245,158,11,0.1)", desc: "Fast sales processing" },
    { id: "staff", label: "Staff & Access", icon: <Users size={16} />, color: "#EF4444", bg: "rgba(239,68,68,0.1)", desc: "Roles, CRUD & attendance" },
    { id: "auth", label: "Auth & Security", icon: <Shield size={16} />, color: "#6366F1", bg: "rgba(99,102,241,0.1)", desc: "Login, 2FA, MFA & reset" },
];

function TabDrawer({ activeTab, onSelect, onClose }: { activeTab: string; onSelect: (id: string) => void; onClose: () => void }) {
    const activeData = TABS.find(t => t.id === activeTab)!;
    return (
        <AnimatePresence>
            <>
                <motion.div key="backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                    onClick={onClose} className="fixed inset-0 z-[900] bg-black/50 backdrop-blur-sm" />
                <motion.div key="drawer" initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }}
                    ...
                    <div className="flex items-center justify-between px-5 py-3 border-b" style={{ borderColor: "#F1F5F9" }}>
                        <div className="flex items-center gap-2">
                            ...
                            <span className="font-black text-base" style={{ color: "#0F172A", fontFamily: "'Familjen Grotesk',sans-serif" }}>Choose Feature</span>
                        </div>
                        <button onClick={onClose} className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: "#F1F5F9" }}>
                    ...
                    <div className="px-4 py-3 space-y-2 pb-8">
                        {TABS.map((tab) => {
                            ...
                            return (
                                <button key={tab.id} onClick={() => { onSelect(tab.id); onClose(); }}
                                    className="w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl transition-all text-left"
                                    style={{ background: isActive ? tab.bg : "transparent", border: `1px solid ${isActive ? tab.color + "30" : "#F1F5F9"}` }}>
                                    <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
                                        style={{ background: isActive ? tab.color : "#F8FAFC", color: isActive ? "white" : "#94A3B8" }}>
                                        {tab.icon}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <div className="font-bold text-sm" style={{ color: isActive ? tab.color : "#0F172A" }}>{tab.label}</div>
                                        <div className="text-xs" style={{ color: "#94A3B8" }}>{tab.desc}</div>
                                    </div>
                                    {isActive && <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: tab.color }} />}
                                </button>
                            );
                        })}
                    </div>
...
function FeaturesContent() {
    ...
    return (
      ...
      `}</style>
            ...
            <div style={{ background: "linear-gradient(135deg, #050E1F 0%, #0c1a3a 60%, #050E1F 100%)", paddingTop: "96px", position: "relative", overflow: "hidden" }}>
                ...
                <div className="relative max-w-5xl mx-auto px-4 sm:px-6 pt-12 pb-0">
                    ...
                    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
                        className="hidden lg:flex items-center gap-1 overflow-x-auto mt-10" style={{ scrollbarWidth: "none" }}>
                        {TABS.map((tab) => {
                            const isActive = activeTab === tab.id;
                            return (
                                <button key={tab.id} onClick={() => handleTabChange(tab.id)}
                                    ...
                                    style={{ background: isActive ? "#F8FAFC" : "transparent", color: isActive ? tab.color : "rgba(255,255,255,0.5)", fontFamily: "'Outfit',sans-serif" }}>
                                    <span style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 28, height: 28, borderRadius: 8, background: isActive ? tab.bg : "rgba(255,255,255,0.06)", color: isActive ? tab.color : "rgba(255,255,255,0.4)", transition: "all 0.2s" }}>{tab.icon}</span>
                                    {tab.label}
                                </button>
                            );
                        })}
                    ...
                    {/* Mobile tab pill + drawer trigger */}
                    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
                        ...
                        <button onClick={() => setDrawerOpen(true)}
                            ...
                            style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.12)", color: "rgba(255,255,255,0.75)", fontFamily: "'Outfit',sans-serif" }}>
                            <Menu size={15} />
            ...
            {drawerOpen && (
                <TabDrawer activeTab={activeTab} onSelect={handleTabChange} onClose={() => setDrawerOpen(false)} />
            )}
...
// Suspense wrapper — required because FeaturesContent uses useSearchParams
